...
const TEMP_BASE = join(tmpdir(), 'axion-integration-tests');

/** Test API URL - defaults to local server */
export const TEST_API_URL = process.env.AXION_API_URL || 'http://localhost:3000';
    ...
    const context: TestProject = {
        ...
        async run(...args: string[]): Promise<CliResult> {
            const result = await execa('node', [CLI_PATH, ...args], {
                ...
                env: {
                    ...process.env,
                    AXION_API_URL: TEST_API_URL,
                    // Clear any local credentials for clean tests
                    HOME: dir,
                    AXION_TOKEN: process.env.AXION_TEST_TOKEN || '',
                },
                all: true,
                reject: true,
            });
...
/**
 * Asserts that test server is running, otherwise skips test
 */
export async function requireTestServer(): Promise<void> {
    const running = await isTestServerRunning();
    if (!running) {
        throw new Error(
            `Test server not running at ${TEST_API_URL}. ` +
            'Start with: npm run dev:server (in platform repo)'
        );
    }
}
